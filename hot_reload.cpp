...
HotReload::HotReload(const std::vector<std::string>& initial_paths, StringFileReloadCallback string_callback)
    ...
    initial_scan(); // Populate initial modification times
}
...
void HotReload::initial_scan() {
    // This function is called by constructors AFTER paths are added via add_file_to_watch
    // add_file_to_watch already stores the initial last_write_time.
    // This function can be used for any other one-time setup if needed,
    // or to trigger an initial load if desired (current design does not do that).
    // For now, it's mostly a placeholder for clarity.
...
void HotReload::poll_changes() {
    for (const auto& path_str : _watched_file_paths) {
        ...
        if (!std::filesystem::exists(fs_path)) {
            // File might have been deleted. For simplicity, we'll just skip it.
            // More robust handling might involve a "deleted" callback or removing it.
            // If it reappears, last_write_time check will handle it.
            continue; 
        }
        ...
        try {
            ...
            if (it != _watched_files_info.end()) {
                ...
                // This case should ideally not happen if add_file_to_watch is used correctly.
                // But as a fallback, treat it as a new file to track.
            ...
            // Optionally, remove the file from watching here if it's consistently problematic
            // remove_file_from_watch(path_str); 
            // Be careful with modifying the container while iterating; use a copy or defer removal.
            // For simplicity now, just log and continue.
        }
    }
}
