...
enum class UnaryOperatorKind
{
    ...
    PreIncrement = (int)TokenKind::Increment,
    PreDecrement = (int)TokenKind::Decrement,
...
struct Token {
    ...
    KeywordKind to_keyword_kind() const {
        auto casted = magic_enum::enum_cast<KeywordKind>(static_cast<int>(kind)).value_or(KeywordKind::Invalid); assert(casted != KeywordKind::Invalid);
        return casted;
    }

    UnaryOperatorKind to_unary_operator_kind() const {
        auto casted = magic_enum::enum_cast<UnaryOperatorKind>(static_cast<int>(kind)).value_or(UnaryOperatorKind::Invalid); assert(casted != UnaryOperatorKind::Invalid);
        return casted;
    }

    BinaryOperatorKind to_binary_operator_kind() const {
        auto casted = magic_enum::enum_cast<BinaryOperatorKind>(static_cast<int>(kind)).value_or(BinaryOperatorKind::Invalid); assert(casted != BinaryOperatorKind::Invalid);
        return casted;
    }

    AssignmentOperatorKind to_assignment_operator_kind() const {
        auto casted = magic_enum::enum_cast<AssignmentOperatorKind>(static_cast<int>(kind)).value_or(AssignmentOperatorKind::Invalid); assert(casted != AssignmentOperatorKind::Invalid);
        return casted;
    }

    ModifierKind to_modifier_kind() const {
        auto casted = magic_enum::enum_cast<ModifierKind>(static_cast<int>(kind)).value_or(ModifierKind::Invalid); assert(casted != ModifierKind::Invalid);
        return casted;
    }

    LiteralKind to_literal_kind() const {
        auto casted = magic_enum::enum_cast<LiteralKind>(static_cast<int>(kind)).value_or(LiteralKind::Invalid); assert(casted != LiteralKind::Invalid);
        return casted;
    }
