...
#include "../common/script_token_types.hpp" // Updated path
#include "../common/script_error.hpp" // Updated path
#include <string_view>
#include <vector>
#include <optional>
#include <variant>
...
// struct CompilationUnitNode; // script_ast.hpp should provide this now.

// SourceLocation and ParseError are included via script_error.hpp -> ast_location.hpp
// TokenType is included via script_token_types.hpp
...
private:
    ...
    std::shared_ptr<DestructorDeclarationNode> parse_destructor_declaration(
        const SourceLocation& decl_start_loc,
        std::vector<std::pair<ModifierKind, std::shared_ptr<TokenNode>>> modifiers,
        const CurrentTokenInfo& tilde_token_info);

    std::shared_ptr<ExternalMethodDeclarationNode> parse_external_method_declaration();
    std::shared_ptr<MemberDeclarationNode> parse_member_declaration();
    ...
    std::shared_ptr<ExpressionNode> parse_conditional_expression();
