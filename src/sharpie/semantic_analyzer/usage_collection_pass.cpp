...
void SemanticAnalyzer::collect_usages_and_type_check(std::shared_ptr<AstNode> node) {
    ...
    if (auto cu = std::dynamic_pointer_cast<CompilationUnitNode>(node))         return collect_usages_and_type_check(cu);
    if (auto ns = std::dynamic_pointer_cast<NamespaceDeclarationNode>(node))    return collect_usages_and_type_check(ns);
    if (auto cd = std::dynamic_pointer_cast<ClassDeclarationNode>(node))        return collect_usages_and_type_check(cd);
...
void SemanticAnalyzer::analyze_statement(std::shared_ptr<LocalVariableDeclarationStatementNode> node) {
    ...
    for (const auto& declarator : node->declarators) {
        if (!declarator || !declarator->name) continue;
...
void SemanticAnalyzer::analyze_statement(std::shared_ptr<IfStatementNode> node) {
    ...
    if (node->thenStatement) analyze_statement(node->thenStatement);
    if (node->elseStatement.has_value()) analyze_statement(node->elseStatement.value());
}
...
void SemanticAnalyzer::analyze_statement(std::shared_ptr<WhileStatementNode> node) {
    ...
    if (node->body) analyze_statement(node->body);
...
SemanticAnalyzer::ExpressionTypeInfo SemanticAnalyzer::analyze_expression(std::shared_ptr<ExpressionNode> node) {
    ...
    if (auto literal = std::dynamic_pointer_cast<LiteralExpressionNode>(node)) return analyze_expression(literal);
    if (auto identifier = std::dynamic_pointer_cast<IdentifierExpressionNode>(node)) return analyze_expression(identifier);
    if (auto binary = std::dynamic_pointer_cast<BinaryExpressionNode>(node)) return analyze_expression(binary);
    if (auto assignment = std::dynamic_pointer_cast<AssignmentExpressionNode>(node)) return analyze_expression(assignment);
    if (auto unary = std::dynamic_pointer_cast<UnaryExpressionNode>(node)) return analyze_expression(unary);
    if (auto method_call = std::dynamic_pointer_cast<MethodCallExpressionNode>(node)) return analyze_expression(method_call);
    if (auto object_creation = std::dynamic_pointer_cast<ObjectCreationExpressionNode>(node)) return analyze_expression(object_creation);
    if (auto this_expr = std::dynamic_pointer_cast<ThisExpressionNode>(node)) return analyze_expression(this_expr);
    if (auto cast = std::dynamic_pointer_cast<CastExpressionNode>(node)) return analyze_expression(cast);
    if (auto member_access = std::dynamic_pointer_cast<MemberAccessExpressionNode>(node)) return analyze_expression(member_access);
    if (auto parenthesized = std::dynamic_pointer_cast<ParenthesizedExpressionNode>(node)) return analyze_expression(parenthesized);
...
SemanticAnalyzer::ExpressionTypeInfo SemanticAnalyzer::analyze_expression(std::shared_ptr<LiteralExpressionNode> node) {
    if (!node) return ExpressionTypeInfo{};
    switch (node->kind) {
        case LiteralKind::Integer: return ExpressionTypeInfo{create_primitive_type("int")};
        case LiteralKind::Long:    return ExpressionTypeInfo{create_primitive_type("long")};
        case LiteralKind::Float:   return ExpressionTypeInfo{create_primitive_type("float")};
        case LiteralKind::Double:  return ExpressionTypeInfo{create_primitive_type("double")};
        case LiteralKind::Boolean: return ExpressionTypeInfo{create_primitive_type("bool")};
        case LiteralKind::Char:    return ExpressionTypeInfo{create_primitive_type("char")};
        case LiteralKind::String:  return ExpressionTypeInfo{create_primitive_type("string")};
        case LiteralKind::Null:    return ExpressionTypeInfo{create_primitive_type("null")};
        default: add_error("Unknown literal kind", node->location); return ExpressionTypeInfo{};
    }
}

SemanticAnalyzer::ExpressionTypeInfo SemanticAnalyzer::analyze_expression(std::shared_ptr<IdentifierExpressionNode> node) {
    if (!node || !node->identifier) return ExpressionTypeInfo{};
    ...
    if (auto* var = ir->symbol_table.find_variable(name)) {
        ...
        // Record a read usage. This might be corrected to a write by an assignment expression.
    ...
    if (auto* extern_func = ir->symbol_table.find_method(name)) {
        if (extern_func->is_external) {
            record_usage(name, UsageKind::Read, node->location); // Reference to a function pointer
...
SemanticAnalyzer::ExpressionTypeInfo SemanticAnalyzer::analyze_expression(std::shared_ptr<BinaryExpressionNode> node) {
    if (!node || !node->left || !node->right) return ExpressionTypeInfo{};
    
    ExpressionTypeInfo left = analyze_expression(node->left);
    ExpressionTypeInfo right = analyze_expression(node->right);

    if (!left.type || !right.type) return ExpressionTypeInfo{};

    switch (node->opKind) {
        case BinaryOperatorKind::Add:
            if (is_string_type(left.type) || is_string_type(right.type)) return ExpressionTypeInfo{create_primitive_type("string")};
            if (is_numeric_type(left.type) && is_numeric_type(right.type)) return ExpressionTypeInfo{promote_numeric_types(left.type, right.type)};
            break;
        ...
        case BinaryOperatorKind::NotEquals:
            if (are_types_compatible(left.type, right.type)) return ExpressionTypeInfo{create_primitive_type("bool")};
            break;
        ...
        case BinaryOperatorKind::GreaterThanOrEqual:
            if (is_numeric_type(left.type) && is_numeric_type(right.type)) return ExpressionTypeInfo{create_primitive_type("bool")};
            break;

        case BinaryOperatorKind::LogicalAnd:
        case BinaryOperatorKind::LogicalOr:
            if (is_bool_type(left.type) && is_bool_type(right.type)) return ExpressionTypeInfo{create_primitive_type("bool")};
            break;
    }
...
SemanticAnalyzer::ExpressionTypeInfo SemanticAnalyzer::analyze_expression(std::shared_ptr<AssignmentExpressionNode> node) {
    if (!node || !node->target || !node->source) return ExpressionTypeInfo{};
...
SemanticAnalyzer::ExpressionTypeInfo SemanticAnalyzer::analyze_expression(std::shared_ptr<UnaryExpressionNode> node) {
    ...
    if (!operand_info.type) return {}; // Error already reported
...
SemanticAnalyzer::ExpressionTypeInfo SemanticAnalyzer::analyze_expression(std::shared_ptr<MethodCallExpressionNode> node) {
    if (!node || !node->target) return ExpressionTypeInfo{};

    ExpressionTypeInfo target_info;
    std::string method_name;
    bool is_static_call_on_type = false;

    // Step 1: Resolve the target of the call to determine if it's an instance, a static type, or an implicit call.
    if (auto member_access = std::dynamic_pointer_cast<MemberAccessExpressionNode>(node->target)) {
        ...
        if (target_info.class_info && !target_info.is_lvalue) is_static_call_on_type = true;
        ...
        // If not extern, it's an implicit call within the current class.
        if (context->currentClassName.empty()) { add_error("Cannot make implicit call to '" + method_name + "' outside of a class context.", node->location); return {}; }

        target_info.class_info = &ir->symbol_table.find_class(context->currentClassName)->type_info;
        if (context->inStaticMethod) {
            is_static_call_on_type = true;
        }
    } else { add_error("Unsupported method call target.", node->target->location); return {}; }
    
    // Step 2: Validate the call using the resolved target information.
    if (!target_info.class_info) { add_error("Attempt to call method '" + method_name + "' on a non-class type.", node->target->location); return {}; }

    auto* method_symbol = ir->symbol_table.find_method_in_class(target_info.class_info->name, method_name);
    if (!method_symbol) { add_error("Method '" + method_name + "' not found in class '" + target_info.class_info->name + "'.", node->target->location); return {}; }
    
    if (is_static_call_on_type && !method_symbol->is_static) { add_error("An object reference is required for the non-static method '" + method_name + "'.", node->target->location); return {}; }
...
SemanticAnalyzer::ExpressionTypeInfo SemanticAnalyzer::analyze_expression(std::shared_ptr<ThisExpressionNode> node) {
    if (context->inStaticMethod) { add_error("'this' cannot be used in a static method.", node->location); return {}; }
    if (context->currentClassName.empty()) { add_error("'this' can only be used within a class.", node->location); return {}; }

    auto* class_symbol = ir->symbol_table.find_class(context->currentClassName);
    if (!class_symbol) { add_error("Internal Error: 'this' used but current class not found.", node->location); return {}; }
    
    return { create_primitive_type(context->currentClassName), &class_symbol->type_info, false };
}

SemanticAnalyzer::ExpressionTypeInfo SemanticAnalyzer::analyze_expression(std::shared_ptr<CastExpressionNode> node) {
    if (!node || !node->targetType || !node->expression) return {};
...
SemanticAnalyzer::ExpressionTypeInfo SemanticAnalyzer::analyze_expression(std::shared_ptr<MemberAccessExpressionNode> node) {
    if (!node || !node->target || !node->memberName) return {};

    ExpressionTypeInfo target_info = analyze_expression(node->target);
    if (!target_info.type || !target_info.class_info) { return {}; }
...
bool SemanticAnalyzer::are_types_compatible(std::shared_ptr<TypeNameNode> left, std::shared_ptr<TypeNameNode> right) {
    ...
    if (left_name == "unknown" || right_name == "unknown") return false;
    if (left_name == right_name) return true;
    
    if ((left_name == "string" || ir->symbol_table.find_class(left_name)) && right_name == "null") return true;
