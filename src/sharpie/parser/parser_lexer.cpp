...
CurrentTokenInfo ScriptParser::lex_operator_or_punctuation()
{
    ...
    {
    case '+':
        if (c2 == '=') { consume_char(); token_info.type = TokenType::PlusAssign; }
        else if (c2 == '+') { consume_char(); token_info.type = TokenType::Increment; }
        else { token_info.type = TokenType::Plus; }
        break;
    case '-':
        if (c2 == '=') { consume_char(); token_info.type = TokenType::MinusAssign; }
        else if (c2 == '-') { consume_char(); token_info.type = TokenType::Decrement; }
        else { token_info.type = TokenType::Minus; }
        break;
    case '*':
        if (c2 == '=') { consume_char(); token_info.type = TokenType::AsteriskAssign; }
        else { token_info.type = TokenType::Asterisk; }
        break;
    case '/':
        if (c2 == '=') { consume_char(); token_info.type = TokenType::SlashAssign; }
        else { token_info.type = TokenType::Slash; }
        break;
    case '%':
        if (c2 == '=') { consume_char(); token_info.type = TokenType::PercentAssign; }
        else { token_info.type = TokenType::Percent; }
        break;
    case '=':
        if (c2 == '=') { consume_char(); token_info.type = TokenType::EqualsEquals; }
        else { token_info.type = TokenType::Assign; }
        break;
    case '!':
        if (c2 == '=') { consume_char(); token_info.type = TokenType::NotEquals; }
        else { token_info.type = TokenType::LogicalNot; }
        break;
    case '<':
        if (c2 == '=') { consume_char(); token_info.type = TokenType::LessThanOrEqual; }
        else { token_info.type = TokenType::LessThan; }
        break;
    case '>':
        if (c2 == '=') { consume_char(); token_info.type = TokenType::GreaterThanOrEqual; }
        else { token_info.type = TokenType::GreaterThan; }
        break;
    case '&':
        if (c2 == '&') { consume_char(); token_info.type = TokenType::LogicalAnd; }
        else { record_error("Unexpected character '&'. Did you mean '&&'?", token_info.location); }
        break;
    case '|':
        if (c2 == '|') { consume_char(); token_info.type = TokenType::LogicalOr; }
        else { record_error("Unexpected character '|'. Did you mean '||'?", token_info.location); }
        break;
    case '(': token_info.type = TokenType::OpenParen; break;
    case ')': token_info.type = TokenType::CloseParen; break;
    case '{': token_info.type = TokenType::OpenBrace; break;
    case '}': token_info.type = TokenType::CloseBrace; break;
    case '[': token_info.type = TokenType::OpenBracket; break; 
    case ']': token_info.type = TokenType::CloseBracket; break; 
    case ';': token_info.type = TokenType::Semicolon; break;
    case ',': token_info.type = TokenType::Comma; break;
    case '.': token_info.type = TokenType::Dot; break;
    case '~': token_info.type = TokenType::Tilde; break;
    default:
