...
std::unique_ptr<CompiledModule> Compiler::compile(const std::vector<SourceFile>& source_files)
{
    ...
    for (size_t i = 0; i < source_files.size(); ++i) {
        ...
        state.ast = state.parser->parse()->as<CompilationUnitNode>();
        ...
        for (const auto& error : state.parser->get_errors()) {
    ...
    for (size_t i = 0; i < file_states.size(); ++i) {
        ...
        DeclarationCollector collector(*state.symbolTable);
    ...
    for (const auto& error : codegen.get_errors()) {
        all_errors.push_back("Codegen: " + error);
    }
